...
var anim_nvl_image_in: Callable = NO_ANIM
...
func set_theme(theme_id: String):
	...
	if animations != null:
		...
		var nvl_image_in = Callable(animations, 'nvl_image_in')
		anim_nvl_image_in = nvl_image_in if nvl_image_in.is_valid() else NO_ANIM
	else:
		anim_overlay_in = NO_ANIM
		anim_overlay_out = NO_ANIM
		anim_shadow_in = NO_ANIM
		anim_shadow_out = NO_ANIM
		anim_nvl_image_in = NO_ANIM
